...

{
    ...
    public void onGlobalLayout() {
        if (once) {
            ...
            mVerticalPadding = Math.max(0, (getHeight() - Utils.INSTANCE.getHeight(widthProportion, heightProportion, getWidth())) / 2);
    ...
    public Bitmap clip() {
        ...
        // 边界检查，确保裁剪参数有效
        int x = Math.max(0, mHorizontalPadding);
        int y = Math.max(0, mVerticalPadding);
        int width = Math.min(getWidth() - 2 * mHorizontalPadding, getWidth() - x);
        int height = Math.min(Utils.INSTANCE.getHeight(widthProportion, heightProportion, getWidth()), getHeight() - y);
